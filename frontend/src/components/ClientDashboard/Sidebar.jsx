...
import gigs from "../../assets/gigs.png";
...
const Sidebar = () => {
    ...
    return (
        <div className="flex">
            ...
            >
                {isOpen ? <IoMdClose className="w-6 h-6" /> : <IoMenu className="w-6 h-6 mt-[-20rem]" />}
            </button>

            {/* Sidebar */}
            <div
                className={`fixed md:relative top-0 left-0 bg-[#0a0f29] text-white w-[312px] h-screen p-5 flex flex-col justify-between overflow-hidden transition-transform duration-300 ${isOpen ? "translate-x-0" : "-translate-x-full md:translate-x-0"
                    }`}
            >
                <div>
                    {/* Logo */}
                    <div className="flex items-center space-x-3 mb-6">
                        ...
                        <h1 className="font-oleo font-normal text-[32px] leading-[44.26px] text-[#f3f3f3]">Blockgigs</h1>
                    </div>
                    ...
                    <ul className="space-y-4">
                        {menuItems.map((item, index) => (
                            <li key={index}>
                                ...
                                >
                                    <img src={item.icon} alt={item.name} className="w-[25px] h-[24px]" />
                                    <span>{item.name}</span>
                                </Link>
                            </li>
                        ))}
